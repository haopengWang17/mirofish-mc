    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Monte Carlo 模式切换 -->
          <div v-if="phase >= 4" class="mc-toggle-row">
            <label class="mc-toggle">
              <input type="checkbox" v-model="mcEnabled" class="mc-checkbox" />
              <span class="mc-toggle-label">{{ $t('monteCarlo.enableMC') }}</span>
            </label>
            <span v-if="mcEnabled" class="mc-hint" @click="showMCSetup = true">
              {{ $t('monteCarlo.configureRuns', { runs: mcConfig.num_runs }) }} ➝
            </span>
          </div>
          ...
          <div class="action-group dual">
            ...
            <button
              v-else
              class="action-btn primary mc-start"
              :disabled="phase < 4"
              @click="handleStartMonteCarlo"
            >
          ...
          <MonteCarloSetup
            :visible="showMCSetup"
            @close="showMCSetup = false"
            @confirm="handleMCConfigConfirm"
          />
        </div>
      </div>
    </div>
...
import { createMonteCarloGroup, startMonteCarloGroup } from '../api/monteCarlo'
import MonteCarloSetup from './MonteCarloSetup.vue'
import { useRouter } from 'vue-router'

const { t } = useI18n()
const mcRouter = useRouter()
...
const showMCSetup = ref(false)
const mcConfig = ref({ num_runs: 10, max_concurrency: 3, classification_criteria: '' })
...
// Monte Carlo 配置确认
const handleMCConfigConfirm = (config) => {
  mcConfig.value = config
  showMCSetup.value = false
  addLog(t('monteCarlo.configSaved', { runs: config.num_runs }))
}

// 启动 Monte Carlo
const handleStartMonteCarlo = async () => {
  if (!props.simulationId) return

  addLog(t('monteCarlo.creating'))
  try {
    // 1. 创建 MC 组
    const createRes = await createMonteCarloGroup({
      simulation_id: props.simulationId,
      num_runs: mcConfig.value.num_runs,
      max_concurrency: mcConfig.value.max_concurrency,
      classification_criteria: mcConfig.value.classification_criteria,
    })

    if (!createRes?.success) {
      addLog(`Error: ${createRes?.error || 'Unknown'}`)
      return
    }

    const groupId = createRes.data.group_id
    addLog(t('monteCarlo.created', { id: groupId }))

    // 2. 启动 MC 组
    const startParams = { group_id: groupId, platform: 'parallel' }
    if (useCustomRounds.value) {
      startParams.max_rounds = customMaxRounds.value
    }

    const startRes = await startMonteCarloGroup(startParams)
    if (!startRes?.success) {
      addLog(`Error: ${startRes?.error || 'Unknown'}`)
      return
    }

    addLog(t('monteCarlo.started', { runs: mcConfig.value.num_runs }))

    // 3. 导航到 MC 视图
    mcRouter.push({ name: 'MonteCarlo', params: { groupId } })
  } catch (e) {
    addLog(`Monte Carlo error: ${e.message}`)
  }
}
